...
public class Karyawan extends javax.swing.JFrame {
    ...
    public void table() {
        ...
        try {
            ...
            ResultSet rs = st.executeQuery("SELECT * FROM profil JOIN akun "
                    + "ON profil.kd_akun = akun.kd_akun;");
    ...
    private void btnEnterMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnEnterMouseClicked
        ...
        try {
            Connection c = (Connection) Config.configDB();
            String sql =
                    "INSERT INTO `akun`(`kd_akun`, `username`, `pass`) VALUES ('"
                    + kodeAkun +"','"
                    + username +"'," 
                    +"'"
        ...
        try {
            ...
            String sql =
                    "INSERT INTO `profil`(`kd_profil`, `nama`, `role`, `kd_akun`)"
                    + "VALUES ('"+ kodeProfil +
                            "','"+ nama +"','"+ role +"','"+ kodeAkun +"')";
            PreparedStatement p = c.prepareStatement(sql);
            p.executeUpdate();
    ...
    private void btnDeleteMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnDeleteMouseClicked
        ...
        try {
            String sql = "SELECT `kd_profil`, `nama`, `kd_akun`, `role` FROM"
                    + " `profil`"
                    + " WHERE kd_profil = '"
                    + kdProfil +
                    "';";
    ...
    private void btnEditMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnEditMouseClicked
        ...
        try {
            String sql = "SELECT * FROM profil JOIN akun"
                    + " ON profil.kd_akun = akun.kd_akun"
                    + " WHERE kd_profil = '"
                    + kdProfil +
                    "';";
        ...
        try {
            Connection c = (Connection) Config.configDB();
            String sql =
                    "UPDATE `profil` SET `nama`='"+ nama +"',`role`='"+ role +"'"
                    + " WHERE kd_profil = '"+ kdProfil +"';";
            PreparedStatement p = c.prepareStatement(sql);
            p.executeUpdate();
            p.close();
        } catch (Exception e) {
        ...
        try {
            Connection c = (Connection) Config.configDB();
            String sql =
                    "UPDATE `akun` SET `username`='"+ user +"',`pass`='"+ password +"'"
                    + " WHERE kd_akun = '"+ kdAkun +"';";
            PreparedStatement p = c.prepareStatement(sql);
            p.executeUpdate();
